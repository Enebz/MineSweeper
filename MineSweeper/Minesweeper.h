...
public:
  ...
  sf::FloatRect getBoundaries() { return boundaries; }

  bool isAlive() { return alive; }

  bool hasWon() { return won; }

  int getRemainingTiles() { return remainingTiles; }

  // Game functions
  void newGame() { newGame(width, height, bombs); }
  ...
  int getAdjacentBombsCount(Tile &tile) {
    ...
    for (int i = std::max(tile.index.x - 1, 0);
         i <= std::min(width - 1, tile.index.x + 1); i++) {
      for (int j = std::max(tile.index.y - 1, 0);
           j <= std::min(height - 1, tile.index.y + 1); j++) {
  ...
  void onClick(sf::Event::MouseButtonEvent mouseButton) {
    ...
    if (!(mouseButton.x > boundaries.left &&
          mouseButton.x < boundaries.left + boundaries.width) ||
        !(mouseButton.y > boundaries.top &&
          mouseButton.y < boundaries.top + boundaries.height)) {
      return;
    }
    ...
    // Middle click bombfield
      ...
      if (tile.getType() != getAdjacentFlagsCount(tile)) {
        return;
      }
      revealAdjacent(tile);
    }
  ...
  void render(sf::RenderWindow &window) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        Tile tile = getTile(sf::Vector2i(x, y));
        tile.render(window, tileSize,
                    sf::Vector2i(x * tileSize + boundaries.left,
                                 y * tileSize + boundaries.top),
                    textures);
      }
    }
  }
